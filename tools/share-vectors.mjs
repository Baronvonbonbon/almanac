...
function pairingCode(providerKey, firstOpeningKey, name) {
  const n = utf8(name);
  return "ALMANAC:P:" + toBase32(cat(new Uint8Array([VERSION, KIND.pairing]), providerKey, firstOpeningKey, new Uint8Array([n.length]), n));
}
...
function build() {
  ...
  return {
    ...
    pairing: {
      name: "Dr Okafor",
      code: pairingCode(providerPublic, openingPublic, "Dr Okafor"),
    },
